...
#                                                 +#+#+#+#+#+   +#+            #
#    Created: 2020/01/10 17:44:41 by sqatim            #+#    #+#              #
#    Updated: 2020/01/16 22:05:33 by sqatim           ###   ########.fr        #
#                                                                              #
# **************************************************************************** #
...
RUN chown -R www-data:www-data /var/www/* &&\
    chmod -R 755 /var/www/*
RUN service mysql start && \
    mysql -e "GRANT SELECT, INSERT, UPDATE, DELETE ON phpmyadmin.* TO 'samir'@'localhost' IDENTIFIED BY 'password';GRANT ALL PRIVILEGES ON *.* TO 'samir'@'localhost' IDENTIFIED BY 'password' WITH GRANT OPTION;"
RUN mkdir ~/mkcert &&\
    cd ~/mkcert &&\
